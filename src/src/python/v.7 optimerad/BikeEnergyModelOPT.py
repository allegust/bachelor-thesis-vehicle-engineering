...
def simulate_energy(StepDist, StepAngle, StepRRcoef,
    ...
    """
    Optimized version of the MATLAB-aligned simulation.
    Improves efficiency while preserving logic.
    """
    Steps = len(StepDist)
    Time = 0.0
    Energy = 0.0
    ...
    for i in range(Steps):
        ...
        for ll in range(Dist_cm):
            ...
            # Instead of creating a new list: just use lengths
            ...
            redSpdStatus, v_x_target, ax_temp, StopFlag, SpeedReductionTable = Speed_Reduction_Caused_By_CrossRoads(
                ...
                Dist_cm, Steps, StepDist, ll, i,
                v_x_total=[],  # not used in logic now
                v_x_list=[]    # same here
            )
