...
from BikeEnergyModelFINAL_w_ALGO import BikeEnergyModel   # lever‑1 engine
...
def pad_profile(x, z, x_start_ref, x_end_ref, n_pad=10):
    ...
    if x_start > x_start_ref:
        x_pad = np.linspace(x_start_ref, x_start, n_pad, endpoint=False)
        z_pad = np.full_like(x_pad, z[0])
        x_new = np.concatenate([x_pad, x_new])
        z_new = np.concatenate([z_pad, z_new])

    if x_end < x_end_ref:
        x_pad = np.linspace(x_end, x_end_ref, n_pad+1)[1:]
...
MASS  = 70.0      # kg (woman 70 kg, men 85 kg)
C_R   = 0.007
CWXA  = 0.45
