...
if __name__ == "__main__":
    ...
    # Sorted population fraction (from 0 to 1)
    y_women = np.linspace(0, 1, len(Aw_kph))
    y_men = np.linspace(0, 1, len(Am_kph))

    # Plot "average speed" vs "sorted population of cyclists"
    plt.figure(figsize=(10, 6))
    plt.plot(sorted(Aw_kph), y_women, label='Women', color='orange')
    plt.plot(sorted(Am_kph), y_men, label='Men', color='blue')

    plt.xlabel('Average Speed [km/h]')
    plt.ylabel('Sorted Population of Cyclists [1]')
    plt.title('Sorted Population of Cyclists vs Average Speed')
    plt.grid(True)
    plt.legend()
    plt.show()