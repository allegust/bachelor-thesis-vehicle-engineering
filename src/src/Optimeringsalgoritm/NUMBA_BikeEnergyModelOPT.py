...
def MapData(map_file_path_or_coords, FigStatus, RRcoef_input, Ay_max, temp):
    # Ny logik: tillåt direkt koordinatinmatning
...
@njit
def update_velocity_and_energy(vx, ax_temp, step_size, P_in, P_roll, P_air_, P_climb, P_acc_):
    new_vx_sq = vx * vx + 2.0 * ax_temp * step_size
    vx_new = math.sqrt(new_vx_sq) if new_vx_sq > 0.0 else 0.0
    ...
    return vx_new, Time, Energy, Energy_roll, Energy_air, Energy_climb, Energy_acc
...
def simulate_energy(StepDist, StepAngle, StepRRcoef,
    ...
    for i in range(Steps):
        ...
        for ll in range(Dist_cm):
            ...
            Dist_now = (len(v_x_total) + len(v_x_segment)) * step_size
            ...
            if redSpdStatus == 1 or StopFlag == 1:
                ...
                P_in = P_roll = P_air_ = P_climb = P_acc_ = 0.0
            else:
                if step_angle <= Alpha_Vmax_steadystate:
                    if vx > V_max:
                        ...
                        P_in = 0.0
                        ax_temp = Power_Input_Off(m, step_rr, step_angle, vx, cwxA, rho)
                        P_roll = P_air_ = P_climb = P_acc_ = 0.0
                    else:
            ...
            vx, dT, dE, dEr, dEa, dEc, dEa2 = update_velocity_and_energy(
                vx, ax_temp, step_size, P_in, P_roll, P_air_, P_climb, P_acc_
            )
            Time += dT
            Energy += dE
            Energy_roll += dEr
    ...
    Distance = total_cm * step_size
    AvgSpeed = Distance / Time if Time > 0 else 0.0

    return (
        Energy, Time, Distance, AvgSpeed,
